...
function VehicleGetdata() {
  ...
  useEffect(() => {
    axios
      .get('https://proccms-backend.onrender.com/api/vehicles')
  ...
  const results = useMemo(() => {
    ...
    return data.filter((item) =>
      ...
      item.vehicleReg?.toLowerCase().includes(q) ||
      item.vehicleType?.toLowerCase().includes(q) ||
      item.classOrDept?.toLowerCase().includes(q)
    );
  }, [data, query]);
  ...
  return (
    <Container>
      ...
      <Form>
        <Row className="mb-3">
          <Col md={6}>
            <Form.Label>Search (Name, Reg No, Type, Dept)</Form.Label>
            <Form.Control
              type="search"
        ...
        <tbody>
          {results.map((item) => (
            <tr key={item._id}>
              ...
              <td>
                {item.vehicleReg}, {item.vehicleType}
              </td>
              <td>{item.passNo}</td>
              <td>{item.issuedTo}</td>
            </tr>
      ...
      <div className="qr-print-grid">
        <Row xs={1} sm={2} md={3} lg={4} className="g-3">
          {results.map((item) => (
            <Col key={item._id}>
              <Card className="p-3 text-center shadow-sm" onClick={() => openModal(item)}>
                <div id={`qr-code-${item._id}`} className="qr-box">
                  <QRCode value={JSON.stringify(item)} size={130} />
                  <div className="qr-label">
                    Vehicle Reg. No & Type {item.vehicleReg}, {item.vehicleType}
                  </div>
                </div>
                <Card.Body>
                  <Card.Title>{item.name}</Card.Title>
                  <Card.Text style={{ fontSize: '0.85rem' }}>
                    Dept: {item.classOrDept}
                    <br />
                    Pass No: {item.passNo}
                    <br />
