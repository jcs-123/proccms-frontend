...
const Dashboard = () => {
  ...
  const [roomRequests, setRoomRequests] = useState([]); // non-completed from backend
  ...
  const fetchDashboardData = async (customFrom, customTo) => {
    try {
      ...
      const [summaryRes, staffRes, roomRes] = await Promise.all([
        ...
        axios.get("https://proccms-backend.onrender.com/api/admin/room-requests"), // already excludes completed
      ]);
  ...
  const handleCardClick = (type) => {
    if (type === "assigned") {
      ...
      navigate("/booking-calender"); // ✅ navigate to booking calendar
    }
  };
